              ...
              SliverPersistentHeader(
                  ...
                  delegate: PersistentHeaderBuilder(
                      builder: (ctx, offset) => GestureDetector(
                            onTap: () {
                              _xiaoShuoProvider
                                  .setReadList("${_detail.id}_${_detail.lastChapterId}_${_detail.lastChapter}");
              ...
              readLastZj != ""
                  ? SliverToBoxAdapter(
                      child: GestureDetector(
                        onTap: () {
                          int index = _xiaoShuoProvider.readList
                              .lastIndexWhere((element) => element.split("_")[0] == _detail.id);
                    ...
                    children: <Widget>[
                      ...
                      Consumer<XiaoShuoProvider>(
                          builder: (_, provider, __) => TextButton(
                              onPressed: () {
                                if (provider.xiaoshuo.where((element) => element.id == _detail.id).toList().length >
                                    0) {
                                ...
                                child: Consumer<XiaoShuoProvider>(
                                    builder: (_, provider, __) => ListTile(
                                          selected: provider.readList
                                                      .where((element) =>
                                                          element.split("_")[1] ==
                                                          "${_baseListProvider.list[index].id}")
                                                      .toList()
                                                      .length >
                                                  0
                                              ? true
...
class PersistentHeaderBuilder extends SliverPersistentHeaderDelegate {
  ...
  PersistentHeaderBuilder({this.max = 60, this.min = 40, @required this.builder})
